...
public sealed class HookRingReaderTests
{
    ...
    [Fact]
    public void Upload_policy_publishes_the_dedicated_action()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var policy = reader.PublishUploadElisionPolicy(
            TimeSpan.FromSeconds(1),
            actionBudget: 64);

        Assert.Equal(HookRingReader.SkipRedundantUploadCopyAction, policy.ActionMask);
        Assert.Equal(64UL, policy.ActionBudget);
        Assert.Equal(
            (long)HookRingReader.SkipRedundantUploadCopyAction,
            writer.ReadInt64(96));
        Assert.Equal(64, writer.ReadInt64(104));
    }

    [Fact]
    public void Update_subresource_policy_publishes_the_dedicated_action()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var policy = reader.PublishUpdateSubresourceElisionPolicy(
            TimeSpan.FromSeconds(1),
            actionBudget: 64);
        ...
        Assert.Equal(64UL, policy.ActionBudget);
        Assert.Equal(
            (long)HookRingReader.SkipRedundantUpdateSubresourceAction,
            writer.ReadInt64(96));
        Assert.Equal(64, writer.ReadInt64(104));
    }
