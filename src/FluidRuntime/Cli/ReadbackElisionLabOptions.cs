...
public sealed record ReadbackElisionLabOptions(
    string TargetPath,
    string HookPath,
    string OutputPath,
    int TrialPairs,
    ...
    bool UseHardware)
{
    public const int ReadbackBufferBytes = 4 * 1024 * 1024;
    public const int RedundantCopyCount = 64;

    public const string Usage =
        "Usage: fluidruntime readback-elision-lab --target <hook-target.exe> " +
        "--hook <hook.dll> --out <report.json> [--trial-pairs <count>] " +
        "[--warmup-pairs <count>] [--hold-ms <milliseconds>] " +
        "[--gpu-timeout-ms <milliseconds>] [--hardware <true|false>]";

    public static ReadbackElisionLabOptions Parse(string[] args)
    {
